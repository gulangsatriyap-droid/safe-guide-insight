  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-sm animate-fade-in">
      ...
      <div className="p-5 border-b border-border">
        ...
        {/* Info Banner */}
        <div className="mt-4 p-3 rounded-lg bg-info/5 border border-info/20 flex items-start gap-2.5">
          <Info className="w-4 h-4 text-info shrink-0 mt-0.5" />
          <p className="text-sm text-info/80">
            Laporan yang sudah selesai dianalisis AI akan otomatis pindah ke tab <strong className="text-info">Evaluasi → Daftar Laporan</strong> untuk proses evaluasi selanjutnya.
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-shrink-0">
            ...
            <Input 
              ...
              className="pl-9 bg-card border-border h-9 text-sm w-52 rounded-lg"
              value={searchTerm}
              onChange={(e) => {
          ...
          <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setCurrentPage(1); }}>
            <SelectTrigger className="w-[120px] h-9 text-sm bg-card border-border rounded-lg">
          ...
          <Select value={siteFilter} onValueChange={handleSiteChange}>
            <SelectTrigger className="w-[130px] h-9 text-sm bg-card border-border rounded-lg">
          ...
          <Select value={lokasiAreaFilter} onValueChange={handleLokasiAreaChange}>
            <SelectTrigger className="w-[140px] h-9 text-sm bg-card border-border rounded-lg">
          ...
          <Select value={detailLokasiFilter} onValueChange={handleDetailLokasiChange}>
            <SelectTrigger className="w-[160px] h-9 text-sm bg-card border-border rounded-lg">
          ...
          <Button
            ...
            className="h-9 px-3 text-sm gap-1.5 rounded-lg bg-card border-border"
          >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap">ID</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap">Waktu</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap">Pelapor</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap">Site</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap">Lokasi</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap">Detail Lokasi</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap w-[160px]">Ketidaksesuaian</th>
              <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 whitespace-nowrap w-[160px]">Sub-Ketidaksesuaian</th>
              ...
              paginatedReports.map((report, index) => (
                <tr 
                  ...
                  className={`border-b border-border hover:bg-muted/30 transition-colors ${
                    report.aiStatus === "SEDANG_ANALISIS_AI" ? "bg-info/5" : ""
                  }`}
                >
                  <td className="px-4 py-3 font-medium text-foreground text-sm whitespace-nowrap">{report.id}</td>
                  <td className="px-4 py-3 text-muted-foreground text-sm whitespace-nowrap">{formatTimestamp(report.timestamp)}</td>
                  <td className="px-4 py-3 text-foreground text-sm whitespace-nowrap">{report.pelapor}</td>
                  <td className="px-4 py-3 text-foreground text-sm whitespace-nowrap">
                    <span className="px-2 py-1 bg-muted rounded-md text-xs font-medium">{report.site}</span>
                  </td>
                  <td className="px-4 py-3 text-foreground text-sm whitespace-nowrap">{report.lokasiArea || "-"}</td>
                  <td className="px-4 py-3 text-foreground text-sm whitespace-nowrap">{report.detailLokasi}</td>
                  <td className="px-4 py-3 max-w-[160px]">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="text-foreground text-sm truncate block cursor-help">
                          {report.ketidaksesuaian || "-"}
                        </span>
                      </TooltipTrigger>
                      <TooltipContent side="top" className="max-w-xs">
                        <p className="text-sm">{report.ketidaksesuaian || "-"}</p>
                      </TooltipContent>
                    </Tooltip>
                  </td>
                  <td className="px-4 py-3 max-w-[160px]">
                    <Tooltip>
                      ...
                      <TooltipContent side="top" className="max-w-xs">
                        <p className="text-sm">{report.subKetidaksesuaian || "-"}</p>
                      </TooltipContent>
                    </Tooltip>
                  </td>
      ...
      <div className="px-5 py-4 border-t border-border flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Menampilkan {filteredAndSortedReports.length > 0 ? startIndex + 1 : 0}-{Math.min(startIndex + ITEMS_PER_PAGE, filteredAndSortedReports.length)} dari {filteredAndSortedReports.length} laporan
        </p>
