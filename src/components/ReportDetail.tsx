import { ArrowLeft, ChevronLeft, ChevronRight, Sparkles, ZoomIn, Clock, CheckCircle2, AlertCircle, RotateCcw, MapPin, ExternalLink, X, Lock } from "lucide-react";
...
const labelConfig = {
  TBC: { bg: "bg-primary", text: "text-primary-foreground", border: "border-primary/30", fullName: "TBC" },
  PSPP: { bg: "bg-amber-500", text: "text-white", border: "border-amber-400/30", fullName: "PSPP" },
  GR: { bg: "bg-slate-600", text: "text-white", border: "border-slate-500/30", fullName: "GR" }
};
...
const ReportDetail = ({ report, onBack, currentIndex, totalReports, onNavigate }: ReportDetailProps) => {
  ...
  // Get source by type or create placeholder
  const getSourceByType = (type: 'TBC' | 'PSPP' | 'GR'): AIKnowledgeSource | null => {
    return aiSources.find(s => s.type === type) || null;
  };

  return (
    <div className="animate-fade-in p-6 overflow-y-auto h-full bg-background">
      {/* Breadcrumb */}
          ...
          {report.labels?.map((label) => {
            ...
            return (
              <Badge 
                key={label}
                variant="outline"
                className={`px-2 py-0.5 text-xs font-medium border-border text-muted-foreground`}
              >
                {config.fullName}
              </Badge>
            );
          ...
          <div className="bg-card rounded-xl p-4 border border-border shadow-sm">
            ...
            <div className="flex items-center gap-2">
              {(['TBC', 'PSPP', 'GR'] as const).map((type) => {
                const isActive = report.labels?.includes(type);
      ...
      {/* Bottom Section - AI Labeled Cards */}
      <div className="mt-5">
        <div className="flex items-center gap-2 mb-3">
          <Sparkles className="w-4 h-4 text-primary" />
          <h3 className="font-semibold text-foreground text-sm">AI Labeled</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(['TBC', 'PSPP', 'GR'] as const).map((type) => {
            ...
            // Always show the card - if no specific source, use defaultAISources
            const defaultSource = defaultAISources.find(s => s.type === type);
            const displaySource = source || defaultSource;
            
            if (displaySource) {
              return <AIAnalysisCard key={type} source={displaySource} isActive={true} />;
            }
